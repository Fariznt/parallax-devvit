 ...
 */
function allHelper({ evalState, nodeList, negate, nodeAddress, evalNode, doEarlyExit, }) {
    const origState = structuredClone(evalState);
...
function anyHelper({ evalState, nodeList, negate, nodeAddress, evalNode, }) {
    let onePassed = false;
    const origState = structuredClone(evalState);
    for (let i = 0; i < nodeList.length; i++) {
        const p = nodeList[i];
    ...
    if (newDefChecks > 1) {
        throw new Error(`1> computation-deferring nodes are not permitted downstream an 'any_of' node
...
export function evalAllOf({ evalState, policyNode, negate, nodeAddress, evalNode, doEarlyExit, }) {
...
export function evalAnyOf({ evalState, policyNode, negate, nodeAddress, evalNode, doEarlyExit }) {
...
export function evalNot({ evalState, policyNode, negate, nodeAddress, evalNode, }) {
    ...
    const origChecks = structuredClone(evalState.deferredChecks);
