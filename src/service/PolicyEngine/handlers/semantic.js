...
export function evalSemantic({ evalState, policyNode, negate, nodeAddress, }) {
 ...
 * @returns
 */
function buildSystemPrompt(checks) {
    const conditions = JSON.stringify(getRuleJson(checks));
    return (`For each of the conditions in the following conditions JSON, 
 ...
 */
async function fetchLLMResponse(apiKey, url, model, context, systemPrompt, text) {
    const messages = [
 ...
 * @returns
 */
function safeParse(rawResponse, model, baseUrl) {
    try {
        ...
        const suffix = trimmed.length > max ? trimmed.slice(Math.max(0, trimmed.length - 200)) : "";
        const details = [
 ...
 * final outputs.
 */
export function validateResponseShape(data) {
    const fail = () => {
        throw new Error(`LLM formatted the JSON incorrectly.\n\nJSON:\n${JSON.stringify(data, null, 2)}`);
    };
    ...
    if (!("results" in d) || !Array.isArray(d.results))
        fail();
    const results = d.results; // safe after Array.isArray check
 ...
 */
function errorOnRemaining(checks) {
    for (const [idStr, check] of Object.entries(checks)) {
        if (check.nodeTrace.result == "deferred") {
            throw new Error(`Failed to evaluate all deferred checks evaluated by model.
        Use a more intelligent model or reduce the number of semantic_checks.`);
        }
    }
}
export async function doDeferredChecks(evalState, contextList, // usually, thread history
targetText, baseUrl, model, key) {
    // let checks: DeferredCheck[] = evalState.deferredChecks
    const checks = Object.fromEntries(evalState.deferredChecks.map((v, i) => [i + 1, v]));
    ...
    // Fetch response
    const rawResponse = await fetchLLMResponse(key, baseUrl, model, context, systemPrompt, target);
    // Parse and validate
    let parsed = safeParse(rawResponse, model, baseUrl);
