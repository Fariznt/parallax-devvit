export function normalize(policy) {
 ...
 */
export function recursiveNorm(policy, inheritedSeverity) {
    ...
    if (policy.next_check)
        recursiveNorm(policy.next_check, effective);
    if ("any_of" in policy) {
        for (const child of policy.any_of)
            recursiveNorm(child, effective);
        return;
    }
    if ("all_of" in policy) {
        for (const child of policy.all_of)
            recursiveNorm(child, effective);
        return;
    }
    if ("not" in policy) {
