const sendModmail = async (result, content, context) => {
    ...
    const subject = `Review ${content.type} for ${result.violations.length} failed policy condition(s)`;
    let body = `Review the following ${content.type} by u/${content.username}:  
    ${content.link}\n\n`;
    body += "\`\`\`";
...
// };
const remove = async (result, content, context) => {
 ...
 * @param errorMessage
 */
export async function modmailErr(context, errorMessage, subject = "Policy-Agent Error") {
