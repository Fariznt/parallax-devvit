...
import { loadActionMapFromSettings, loadEarlyExitFromSettings, loadIgnoreFailuresFromSettings, loadKeyFromSettings, loadPolicyFromSettings, } from "./settings-loader.js";
...
Devvit.addSettings([
    ...
    this will remove the related modmail when an item is approved in the custom modqueue webview.`,
        type: 'boolean',
        defaultValue: false,
        scope: 'installation',
    },
...
async function getEngine(context) {
    if (!engine) {
        const policyJson = await loadPolicyFromSettings(context);
...
async function getThread(context, comment) {
    let current = comment;
    const chain = [];
 ...
 * @param context
 */
async function resultApply(result, contentInfo, context) {
    ...
    const actionMap = await loadActionMapFromSettings(context);
    ...
    if (!actionMap || maxSeverity == null) {
        ...
        const actions = actionMap[maxSeverity];
        if (!(maxSeverity in actionMap)) {
            await modmailErr(context, `A severity level defined in a violated Policy does not exist in actionMap`);
        }
        else {
 ...
 */
async function safeEvaluate({ context, contentInfo, contextList, apiKey, doEarlyExit, ignoreFailures, }) {
    try {
        ...
        const status = err instanceof Error && "status" in err && typeof err.status === "number"
            ...
            : null;
        if (status === 503) {
            console.warn(`LLM unavailable (503) while evaluating ${contentInfo.type} at ${contentInfo.link}`);
            if (!ignoreFailures) {
                const link = contentInfo.link ?? "<link could not be obtained>";
                await modmailErr(context, `This ${contentInfo.type} could not be automatically evaluated because the LLM service is temporarily unavailable due to high server load.
        Please review it manually:
        ${link}`, "Unable to evaluate: manual review needed");
            }
            return null;
        }
        // In case of failure, send informative error to modmail.
        await modmailErr(context, `An error occurred trying to evaluate the ${contentInfo.type} at:
    ${contentInfo.link}
...
async function handleCommentCreate(event, context) {
    const comment = event.comment;
    const text = comment?.body ?? "";
    if (!comment || !text)
        return;
    const commentThread = await getThread(context, comment);
    const commentInfo = {
...
async function handlePostCreate(event, context) {
